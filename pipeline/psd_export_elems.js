...
//	Steven An, 2012 - based on example scripts from Adobe
//	Run from Photoshop: File | Scripts | Browse, select this JS file
//----------------------------------------

// enable double clicking from the Macintosh Finder or the Windows Explorer
// Needs to be named .JSX for this to work..but I don't quite trust it yet
#target photoshop
...
function is_text_hook( layername )
{
	return( layername.indexOf( 'ph_' ) == 0 );
}

function is_hooked_layer( layername )
{
	return layername.indexOf('_up') > -1
		|| layername.indexOf('_down') > -1
		|| layername == 'start'
		|| layername == 'results'
		|| layername == 'bg'
		|| layername == 'goodjob'
		|| layername == 'perfect'
		|| layername == 'whatyoumissed'
		|| layername == 'readytotryagain'
		|| layername == 'perfect_popdown'
		|| layername == 'plus5'
		|| layername == 'plus10'
		|| layername == 'start_arrow2'
		|| layername == 'credits'
		|| layername == 'welcome'
		|| layername == 'sound_on'
		|| layername == 'sound_off'
...
function is_pieces_group( layername )
{
	return layername == 'placed_elements';
}

// UI strings to be localized
var strTitle = localize("$$$/JavaScripts/ExportLayersToFiles/Title=Geomatch Export");
...
function write_uibuilder_as( export_state, filename, endl )
{
	var f = new File( filename );
	f.open('w');
	f.write("package geohero {"+endl);
	f.write("import mx.core.UIComponent;"+endl);
	f.write("import flash.display.*;"+endl);
	f.write("import mx.controls.Text;"+endl);
	f.write("import flash.geom.Rectangle;"+endl);
	f.write("import flash.geom.Point;"+endl);
	f.write("public class UIBuilder {"+endl);
	f.write( export_state.res_code + endl );
	f.write( export_state.hook_vars + endl );
	f.write("public var root_container : UIComponent = new UIComponent();"+endl);
	f.write("public var pieces : Array = new Array();"+endl);
	f.write("public function build() : void {"+endl);
	f.write( export_state.build_code+endl );
	f.write("}"+endl);
	f.write("}"+endl);
	f.write("}"+endl);
	f.close();
...
//  
//----------------------------------------
function write_layers_text( exp, filename )
{
	var f = new File( filename );
	f.open('w');

	for( var i = 0; i < exp.relative_file_basepaths.length; i++ )
	{
		f.write( exp.relative_file_basepaths[i]+'\n' );
	}
	f.close();
}

// set should be a hashtable where set['path/to/layer/in/set'] == true
function show_layer_set( paths, id_nodes, parent_name_path )
{
	for( var i = 0; i < id_nodes.length; i++ )
	{
		var node = id_nodes[i];
		var layer = activate_layer( node.id );
		var name_path = parent_name_path + '/' + layer.name;

		if( !node.is_leaf )
		{
			// always show folders - otherwise, the children might not be visible
			layer.visible = true;
			show_layer_set( paths, node.kids, name_path );
		}
		else
		{
			// leaf - only show if it's included in the set
			if( paths[ name_path ] == true )
			{
				layer.visible = true;
			}
			else
...
function main()
{
	...
	try
	{
		...
		{
		//----------------------------------------
		//  Basic approach: Create a duplicate of the current document, remember which layers were visible,
		//	hide all layers, then toggle and export layers one by one (that were originally visible)
		//----------------------------------------
			...
			var top_id_nodes = build_layer_id_tree();

			// remember which layers were hidden originally so we don't export them
			var vis_tree = build_layer_vis_tree( top_id_nodes );

			//----------------------------------------
			//  Duplicate
			//----------------------------------------
			var workCopy = app.activeDocument.duplicate();
			workCopy.activeLayer = workCopy.layers[ workCopy.layers.length-1 ]; // for removing

			// make all layers invisible. we'll flip on layers one by one to export
			hide_all_layers( top_id_nodes, vis_tree );

			// always write out the layer name paths to make it easy for the user to create name paths
			var namepathf = new File( exportInfo.destination + '/layer_name_paths.txt' );
			namepathf.open('w');
			save_layer_name_paths( top_id_nodes, namepathf, '' );
			namepathf.close();

			// read in the layer sets
			var layer_sets = load_layer_sets( exportInfo.destination + '/export_layer_sets.txt' );
...
{         
	var LAYER_TYPE_PROP =  stringIDToTypeID("layerSection");
	var ref = new ActionReference(); 
	ref.putIndex(charIDToTypeID( "Lyr " ), id); 
	var desc =  executeActionGet(ref); 
	var type = desc.getEnumerationValue( LAYER_TYPE_PROP ); 
	var res = typeIDToStringID(type); 
	return res;
}; 

function activate_layer( idx )
{ 
...
function DEBUG_get_name( id )
{
	return activate_layer( id ).name
}

function save_layer_name_paths( id_nodes, fout, parent_name_path )
{
	for( var i = 0; i < id_nodes.length; i++ )
	{
		var node = id_nodes[i];
		var layer = activate_layer( node.id );
		var name_path = parent_name_path + '/' + layer.name;
		fout.write( name_path+'\n' );

		if( !node.is_leaf )
			save_layer_name_paths( node.kids, fout, name_path );
	}
}

function load_layer_sets( fname )
{
	...
	while( !f.eof )
	{
		...
		if( state == 'ended' )
		{
			...
			sets.push( curr_set );
			curr_set.name = line;
			curr_set.layer_paths = new Object();
			curr_set.layer_paths.length = 123;
			state = 'inset';
		}
		else if( state == 'inset' )
		{
			...
			{
				// layer name
				curr_set.layer_paths[ line ] = true;
			}
		}
	}
	f.close();
...
function build_layer_id_tree()
{
	var n = get_num_layers()+1;
	var id = n - 1;
	var last_id = 0;
	if( !doc_has_bg() ) last_id = 1;
	...
	function inner()
	{
		...
		while( id >= last_id )
		{
			var type = get_layer_type( id );
			var node = new Object();
			node.id = id;

			if( type == 'layerSectionContent' )
			{
				node.is_leaf = true;
				nodes.push( node );
				id--;
			}
			else if( type == 'layerSectionStart' )
			{
				id--;
				node.is_leaf = false;
				node.kids = inner();
...
function build_layer_vis_tree( id_nodes )
{
	var vis_nodes = new Array();
	...
	for( var i = 0; i < id_nodes.length; i++ )
	{
		...
		visnode.visible = activate_layer( node.id ).visible;

		// only recurse if this group is visible
		if( !node.is_leaf && visnode.visible )
			visnode.kids = build_layer_vis_tree( node.kids );
...
function hide_all_layers( nodes, vis )
{
	for( var i = 0; i < nodes.length; i++ )
	{
		...
		if( !node.is_leaf )
			hide_all_layers( node.kids, vis[i].kids );
	}
}
